...
<Popup trigger={<img src='https://cdn1.iconfinder.com/data/icons/aami-web-internet/64/aami2-36-512.png' className='w-8 h-8' />} modal nested>
  {(close) => (
    <div className='bg-[#ccfbf1] w-80 rounded-lg'>
    <h1 className='font-bold text-center text-xl'>Read Book :</h1>
    <div className='grid grid-cols-3 gap-4 p-4'>
      {Read.map((m,i) => (
...
<Link to={'/'}>
        <button className='p-2 font-bold'>LogOut</button>
        </Link>
      </div>
    </div>
      <div className='bg-[#ccfbf1] p-8 flex flex-row justify-around'>
        <div className='p-4'>
          ...
          <div className='flex flex-row items-center'>
            ...
            />
            <button
              className='rounded-full p-2 border-black font-bold ml-2 text-2xl'
      ...
      <div className='flex justify-center items-center'>
        <div className='grid grid-cols-5 gap-6 max-sm:grid-cols-2 p-2'>
          {books.map((item, index) => (
            <div key={index} className='w-32 flex flex-col items-center p-4 bg-emerald-600/50'>
              <Popup trigger={
              <img src={item.book_image} className='w-32' />
                 } modal nested>
                {(close) => (
                  <div className='bg-[#ccfbf1] w-80 p-4 text-center border rounded-lg'>
                    <div>
                      ...
                      <div>
                        {item.buy_links &&
                          item.buy_links.map((buyItem, indexBuy) => (
                            <div key={indexBuy} className='flex flex-row pl-2'>
                              <p>{buyItem.name}</p>
                              <a
                                href={buyItem.url}
                                className='font-bold'
              ...
              <button onClick={() => {setFevorite([...Favorite,item]) ,localStorage.setItem("itemFavorite", JSON.stringify(Favorite))}}>
                      <img src='https://icons.veryicon.com/png/o/miscellaneous/linear-icon-52/add-favorites.png' className='w-5 h-5'/>
                      </button>
                      <button onClick={() => {setRead([...Read,item]) ,localStorage.setItem("itemRead", JSON.stringify(Read))}}>
                      <img src='https://cdn1.iconfinder.com/data/icons/aami-web-internet/64/aami2-36-512.png' className='w-6 h-6'/>
                      </button>
                      </div>
            </div>
          ))}
